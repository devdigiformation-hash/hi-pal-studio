...
const toolData = OPEN_SOURCE_SUBPAGES["openclaw"];

const TITLE = "Autonomous AI Desktop Operator for Coding, Research & Web Tasks — 100% Free — Free Open-Source Alternative";
const DESC =
      ...
      scripts: [
        {
          ...
          children: JSON.stringify(
            breadcrumbLd([
              ...
              { name: toolData.name, path: "/open-source/openclaw" },
            ]),
          ),
        },
        {
      ...
      <SectionWrapper>
        <Breadcrumbs
          trail={[
            { name: "Home", path: "/" },
            { name: "Open Source", path: "/open-source" },
            { name: toolData.shortName, path: "/open-source/openclaw" },
          ]}
        />
        <div className="mx-auto grid max-w-[1180px] items-center gap-12 lg:grid-cols-[1.05fr_1fr]">
          <div>
            ...
            <h1 className="reveal-item delay-1 mt-3 font-display text-[32px] font-extrabold leading-[1.1] tracking-[-0.035em] text-[var(--text-primary)] md:text-[46px]">
              Autonomous AI Desktop Operator for Coding, Research & Web Tasks —{" "}
            ...
            <div className="reveal-item delay-3 mt-8 flex flex-wrap items-center gap-4">
              ...
              >
                <CyanButton size="lg" icon={<WindowsIcon />}>
                  Download OpenClaw Companion (.EXE)
                </CyanButton>
              </a>
          ...
          <OpenSourceHeroStage
            toolId="openclaw"
            title="OpenClaw Autonomous Operator"
            ...
            quickCommand="git clone https://github.com/digiformation/openclaw && cd openclaw && pip install -r requirements.txt"
            badges={["Local Sandbox","Browser Automation","Human Approval Gate"]}
            annualSavings="Save $1,200 / year vs Manus AI"
            repoUrl={REPO_URL}
            downloadLabel="Download OpenClaw Companion (.EXE)"
          />
        </div>
      </SectionWrapper>
