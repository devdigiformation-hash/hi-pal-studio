...
const toolData = OPEN_SOURCE_SUBPAGES["opencut"];

const TITLE = "Fast, Watermark-Free Social Video Editor for Windows — 100% Free — Free Open-Source Alternative";
const DESC =
      ...
      scripts: [
        {
          ...
          children: JSON.stringify(
            breadcrumbLd([
              ...
              { name: toolData.name, path: "/open-source/opencut" },
            ]),
          ),
        },
        {
...
function OpenCutPage() {
  return (
    <main className="min-h-screen pt-[60px] md:pt-[72px]">
      <SectionWrapper>
        <Breadcrumbs
          trail={[
            { name: "Home", path: "/" },
            { name: "Open Source", path: "/open-source" },
            { name: toolData.shortName, path: "/open-source/opencut" },
          ]}
        />
        <div className="mx-auto grid max-w-[1180px] items-center gap-12 lg:grid-cols-[1.05fr_1fr]">
          <div>
            ...
            <h1 className="reveal-item delay-1 mt-3 font-display text-[32px] font-extrabold leading-[1.1] tracking-[-0.035em] text-[var(--text-primary)] md:text-[46px]">
              Fast, Watermark-Free Social Video Editor for Windows —{" "}
            ...
            <div className="reveal-item delay-3 mt-8 flex flex-wrap items-center gap-4">
              <a
                href={DOWNLOAD_URL}
                download="OpenCut-Setup-x64.exe"
              >
                <CyanButton size="lg" icon={<WindowsIcon />}>
                  Download OpenCut Launcher (.EXE)
                </CyanButton>
              </a>
          ...
          <OpenSourceHeroStage
            toolId="opencut"
            title="OpenCut Social Video Editor"
            category="Video & Creative Media"
            color="#3DDC84"
            downloadUrl={DOWNLOAD_URL}
            downloadFilename="OpenCut-Setup-x64.exe"
            license={toolData.license + " (100% Free)"}
            stars={toolData.stars}
            version="v1.2.0"
            
            quickCommand="git clone https://github.com/opencut/opencut && npm install && npm run dev"
            badges={["Zero Watermarks","Auto-Captions","Vertical 9:16 Reels"]}
            annualSavings="Save $120 / year vs CapCut Pro"
            repoUrl={REPO_URL}
            downloadLabel="Download OpenCut Launcher (.EXE)"
          />
        </div>
      </SectionWrapper>
      ...
      <SectionWrapper>
        <OpenSourceValueComparisonChart toolId="opencut" downloadUrl={DOWNLOAD_URL} />
      </SectionWrapper>
